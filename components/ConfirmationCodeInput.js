...
export default class ConfirmationCodeInput extends Component {
  ...
  _onInputCode(character, index) {
    const { codeLength, onFulfill, compareWithCode, ignoreCase, blurOnFulfill } = this.props;
    let newCodeArr = _.clone(this.state.codeArr);
    ...
    if (index == codeLength - 1) {
      ...
      /**
       * Only blur and dismiss the
       * keyboard when blurOnFulfill
       * is enabled
       */
      if (blurOnFulfill) {
